...
class ArrayUtil {
	...
	public static function encodeHTML ($array) {
		...
		return array_map (function ($element) {
			if (is_array ($element)) return static::encodeHTML ($element); elseif (is_string ($element)) return StringUtil::encodeHTML ($element);
		}, $array);
	}
	...
	public static function encodeUTF8 ($array) {
		...
		return array_map (function ($element) {
			if (is_array ($element)) return static::encodeUTF8 ($element); elseif (is_string ($element)) return StringUtil::encodeUTF8 ($element);
		}, $array);
	}
	...
	public static function toIntegerArray ($array) {
		...
		return array_map (function ($element) {
			if (is_array ($element)) return static::toIntegerArray ($element); elseif (!is_integer ($element)) return intval ($element); // TODO: We should create a util for such methods.
		}, $array);
	}
	...
	public static function trim ($array, $removeEmptyElements = true) {
		...
		$array = array_map (function ($element) {
			if (is_array ($element)) return static::trim ($element); elseif (is_string ($element)) return StringUtil::trim ($element);
		}, $array);
	...
	public static function unifyNewlines ($array) {
		...
		return array_map (function ($element) {
			if (is_array ($element)) return static::unifyNewlines ($element); elseif (is_string ($element)) return StringUtil::unifyNewlines ($element);
		}, $array);
	}
}
