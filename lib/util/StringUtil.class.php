...
class StringUtil {
	...
	public static function getSaltedHash($value, $salt) {
		if (!defined('OPTION_ENCRYPTION_ENABLE_SALTING') || OPTION_ENCRYPTION_ENABLE_SALTING) {
			$hash = '';
			// salt
			if (!defined('OPTION_ENCRYPTION_SALT_POSITION') || OPTION_ENCRYPTION_SALT_POSITION == 'before') {
				$hash .= $salt;
			}

			// value
			if (!defined('OPTION_ENCRYPTION_ENCRYPT_BEFORE_SALTING') || OPTION_ENCRYPTION_ENCRYPT_BEFORE_SALTING) {
			...
			// salt
			if (defined('OPTION_ENCRYPTION_SALT_POSITION') && OPTION_ENCRYPTION_SALT_POSITION == 'after') {
				$hash .= $salt;
			}
	...
	public static function encrypt($value) {
		if (defined('OPTION_ENCRYPTION_METHOD')) {
			switch (OPTION_ENCRYPTION_METHOD) {
	...
	public static function formatDouble($double, $minDecimals = 0) {
		...
		$double = str_replace('.', IKARUS::getLanguage()->get('ikarus.global.decimalPoint'), $double);
	...
	public static function addThousandsSeparator($number) {
		if ($number >= 1000 || $number <= -1000) {
			$number = preg_replace('~(?<=\d)(?=(\d{3})+(?!\d))~', IKARUS::getLanguage()->get('ikarus.global.thousandsSeparator'), $number);
		}
