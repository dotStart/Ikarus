		...
		foreach ($data as $key => $val) {
			switch ($key) {
				case 'authorInformation':
					$instance->setAuthorInformation (PackageAuthorInformation::decode ($val));
					break;
				case 'dependencies':
					$instance->setDependencies (PackageDependency::decode ($val));
					break;
				case 'exclusions':
					$instance->setExclusions (ExcludedPackage::decode ($val));
					break;
				case 'installationInstructions':
					$instance->setInstallationInstructions (InstallationInstructionList::decode ($val));
					break;
				case 'optionalPackages':
					$instance->setOptionalPackages (OptionalPackage::decode ($val));
					break;
				case 'packageInformation':
					$instance->setPackageInformation (UniquePackageInformation::decode ($val));
					break;
				case 'softDependencies':
					$instance->setSoftDependencies (SoftDependency::decode ($val));
					break;
				case 'updateInstructions':
					$instance->setUpdateInstructions (UpdateInstruction::decode ($val));
					break;
				default:
					// fire event
