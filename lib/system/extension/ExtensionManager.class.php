...
class ExtensionManager {
	...
	public function handleOutputBuffer ($buffer) {
		...
		// cancellable event
		if ($event->isCancelled () and $event->getReplacement () === null) throw new StrictStandardException("Missing replacement for output buffer"); elseif ($event->isCancelled ()) return $event->getReplacement ();
		...
		if (Ikarus::getConfiguration ()->get ('global.http.enableGzip')) {
			...
			if (!$event->isCancelled ()) {
				...
				// cancellable event
				if ($event->isCancelled () and $event->getReplacement () === null) throw new StrictStandardException("Missing replacement for gzipped output buffer"); elseif ($event->isCancelled ()) $buffer = $event->getReplacement ();
			}
		}
		...
		// cancellable event
		if ($event->isCancelled () and $event->getReplacement () === null) throw new StrictStandardException("Missing replacement for output buffer"); elseif ($event->isCancelled ()) $buffer = $event->getReplacement ();
