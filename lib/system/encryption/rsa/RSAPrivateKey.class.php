 ...
 * @version		2.0.0-0001
 */
class RSAPrivateKey implements IRSAKey {
	...
	public static function decode($data) {
		...
		// validate
		if (!isset($data->key)) throw new RSAKeyException('Invalid private key supplied: No key found');
		if (!isset($data->modulus))  throw new RSAKeyException('Invalid private key supplied: No modulus found');
		if (!isset($data->keyLength))  throw new RSAKeyException('Invalid private key supplied: No key length defined');
	...
	public function sign($data) {
		$rsa = new RSA(null, $this->key, $this->modulus, $this->keyLength);
		$rsa->sign($data);
	}
}
?>