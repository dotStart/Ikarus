...
class RequestDispatcher extends Singleton {
	...
	public function dispatch (IApplication $application, array $requestParameters) {
		...
		foreach ($this->availableRoutes as $routeParameter => $routes) if (isset($requestParameters[$routeParameter])) foreach ($routes as $routeName => $executionInformation) if ($routeName == $requestParameters[$routeParameter] and $this->loadController ($executionInformation['controllerName'], $executionInformation['controllerNamespace'], $application)) return $this->executeController ($executionInformation['controllerName'], $executionInformation['controllerNamespace'], $application);

		// find controller types
		foreach ($this->availableControllerTypes as $name => $controllerNamespace) if (isset($requestParameters[$name]) and $this->loadController ($requestParameters[$name] . ucfirst ($name), $controllerNamespace, $application)) return $this->executeController ($requestParameters['name'], $controllerNamespace, $application);
	...
	public function executeController ($controllerName, $controllerNamespace, $application) {
		...
		// cancellable event
		if ($event->isCancelled () and $event->getReplacement () === null) throw new StrictStandardException('A replacement for controller execution is missing'); elseif ($event->isCancelled ()) $controller = $event->getReplacement ();
